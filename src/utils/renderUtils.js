...
const classeFrentePeca = (jogador) => {
    if (jogador === 'Vermelho') return 'piece-layout-2 piece-front vermelho';
    if (jogador === 'Azul') return 'piece-layout-2 piece-front azul';
    return 'piece-layout-2';
};

const classeFrenteIcone = (jogador) => {
    if (jogador === 'Vermelho') return 'piece-front vermelho';
    if (jogador === 'Azul') return 'piece-front azul';
    return '';
};

// FUNÇÃO: Renderizar peça com imagem de patente
export const renderPieceWithRank = (numero, jogador = null) => {
    const layoutClass = classeFrentePeca(jogador);
    ...
    const iconClass = classeFrenteIcone(jogador);
    if (iconClass) {
        return <div className={iconClass}>{numero}</div>;
    }
    return numero;
};
